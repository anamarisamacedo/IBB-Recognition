...
class ShallowNN:
    ...
    def get_batch_indices(self, X_train, batch_size):
        #n = X_train.shape[0]
        indices = [range(i, i+batch_size) for i in range(0, len(X_train), batch_size)]
        return indices
    ...
    def fit(self, X_train, y_train, batch_size=32, n_iterations=100, lr=0.01):
        ...
        for i in range(n_iterations):
            for ind in indices:
                print(ind)
                X = X_train[ind[0]:ind[1]]
                print(X_train[ind[0]:ind[1]])
                print(y_train[ind[0]:ind[1]])
                Y = y_train[ind[0]:ind[1]]
